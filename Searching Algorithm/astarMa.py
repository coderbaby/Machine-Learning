#! /usr/bin/python3
'''
Program Title: 8 - Sliding Puzzle Solver (A STAR with Count of manhattan distance as heuristics)
...
def main():
    ...
    result = astarMa(startState, goalState)
    if result == None:
...
def astarMa(startState, goalState):
    fringe = []
...
FUNCTION NAME: f()
   This is a evaluation function 
    ...
    3. h(n) = estimated cost from n to reach goal
       h(n) is the heuristic function using manhattan distance 
...
def f(node, goalState):
    return (node.depth + heuristicManhattan(node.currentState,goalState))


'''

FUNCTION NAME: heuristicManhattan()
      this function calculates sum of manhattan distance of every tile

INPUT PARAMETERS: 
      1. startState - startState is the node's current state,
      2. goalState - goalState is the desired state.

OUTPUT: Sum of Manhattan distance of every misplaced tile

'''
def heuristicManhattan(startState, goalState):
    n = 3
    distance = 0
    matStartState = [startState[i:i+n] for i in range(0, len(startState), n)]
    matGoalState = [goalState[i:i+n] for i in range(0, len(goalState), n)]
    for i in range(n):
        for j in range(n):
            if matStartState[i][j] == 0:
                continue
            distance += abs(i - (matStartState[i][j]/3)) + abs(j -(matStartState[i][j]%3))
        return distance
